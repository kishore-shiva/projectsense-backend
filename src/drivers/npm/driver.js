...
function getJs(page, technologies = projectsense.technologies) {
...
function analyzeJs(js, technologies = projectsense.technologies) {
  return js
...
function getDom(page, technologies = projectsense.technologies) {
  return page.evaluate((technologies) => {
    return technologies
...
function analyzeDom(dom, technologies = projectsense.technologies) {
  return dom
...
class Driver {
  ...
  async open(url, headers = {}, storage = {}) {
    ...
    try {
      const site = new Site(url, headers, this);
...
class Site {
  ...
  async goto(url) {
    ...
    try {
      ...
      if (page.url() === 'about:blank') {
        ...
        error.code = 'projectsense_PAGE_EMPTY'
      ...
      if (error.message.includes('net::ERR_NAME_NOT_RESOLVED')) {
        ...
        newError.code = 'projectsense_DNS_ERROR'
      ...
      if (
        ...
        error.code = 'projectsense_TIMEOUT_ERROR'
      }
  ...
  async onDetect(url, detections = []) {
    ...
    if (this.cache[url.href]) {
      ...
      const requires = [
        ...projectsense.requires.filter(({ name }) =>
          resolved.some(({ name: _name }) => _name === name)
        ),
        ...projectsense.categoryRequires.filter(({ categoryId }) =>
