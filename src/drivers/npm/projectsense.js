...
const benchmarkEnabled =
  typeof process !== 'undefined' ? !!process.env.PROJECTSENSE_BENCHMARK : false
...
const projectsense = {
  ...
  getTechnology: (name) =>
    [
      ...projectsense.technologies,
      ...projectsense.requires.map(({ technologies }) => technologies).flat(),
      ...projectsense.categoryRequires
        .map(({ technologies }) => technologies)
        .flat(),
    ].find(({ name: _name }) => name === _name),

  getCategory: (id) => projectsense.categories.find(({ id: _id }) => id === _id),
  ...
  resolve(detections = []) {
    ...
    projectsense.resolveExcludes(resolved)
    projectsense.resolveImplies(resolved)

    const priority = ({ technology: { categories } }) =>
      categories.reduce(
        (max, id) => Math.max(max, projectsense.getCategory(id).priority),
        0
      )
    ...
    return resolved
      ...
      .map(
        ({
          ...
          description,
          slug,
          categories: categories.map((id) => projectsense.getCategory(id)),
          confidence,
          version,
          icon,
          website,
  ...
  resolveExcludes(resolved) {
    resolved.forEach(({ technology }) => {
      technology.excludes.forEach(({ name }) => {
        const excluded = projectsense.getTechnology(name)
      ...
      resolved.forEach(({ technology, confidence, lastUrl }) => {
        technology.implies.forEach(
          ({ name, confidence: _confidence, version }) => {
            const implied = projectsense.getTechnology(name)
   ...
   * @param {*} param0
   */
  analyze(items, technologies = projectsense.technologies) {
    benchmarks = []

    const oo = projectsense.analyzeOneToOne
    const om = projectsense.analyzeOneToMany
    const mm = projectsense.analyzeManyToMany
  ...
  setTechnologies(data) {
    const transform = projectsense.transformPatterns

    projectsense.technologies = Object.keys(data).reduce((technologies, name) => {
      const {
        cats,
        certIssuer,
        cookies,
      ...
      technologies.push({
        ...
        slug: projectsense.slugify(name),
        text: transform(text),
        url: transform(url),
    ...
    projectsense.technologies
      ...
      .forEach((technology) =>
        technology.requires.forEach(({ name }) => {
          if (!projectsense.getTechnology(name)) {
            throw new Error(`Required technology does not exist: ${name}`)
          }

          projectsense.requires[name] = projectsense.requires[name] || []

          projectsense.requires[name].push(technology)
        })
      )

    projectsense.requires = Object.keys(projectsense.requires).map((name) => ({
      name,
      technologies: projectsense.requires[name],
    }))
    ...
    projectsense.technologies
      ...
      .forEach((technology) =>
        technology.requiresCategory.forEach(({ id }) => {
          projectsense.categoryRequires[id] =
            projectsense.categoryRequires[id] || []

          projectsense.categoryRequires[id].push(technology)
        })
      )

    projectsense.categoryRequires = Object.keys(projectsense.categoryRequires).map(
      (id) => ({
        categoryId: parseInt(id, 10),
        technologies: projectsense.categoryRequires[id],
      })
    )

    projectsense.technologies = projectsense.technologies.filter(
   ...
   */
  setCategories(data) {
    projectsense.categories = Object.keys(data)
      .reduce((categories, id) => {
        const category = data[id]

        categories.push({
          id: parseInt(id, 10),
          slug: projectsense.slugify(category.name),
          ...category,
        })
  ...
  transformPatterns(patterns, caseSensitive = false, isRegex = true) {
    ...
    const parsed = Object.keys(patterns).reduce((parsed, key) => {
      parsed[caseSensitive ? key : key.toLowerCase()] = toArray(
        patterns[key]
      ).map((pattern) => projectsense.parsePattern(pattern, isRegex))
  ...
  parsePattern(pattern, isRegex = true) {
    if (typeof pattern === 'object') {
      return Object.keys(pattern).reduce(
        (parsed, key) => ({
          ...parsed,
          [key]: projectsense.parsePattern(pattern[key]),
        }),
        {}
      )
    } else {
    ...
    return technology[type].reduce((technologies, pattern) => {
      ...
      if (matches) {
        technologies.push({
          ...
          version: projectsense.resolveVersion(pattern, value),
        })
      }
